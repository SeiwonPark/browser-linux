...
//version
window.version = 1.0; //buildroot 2023.2
//parse url
...
if (window.params.has("iso") != true) {
  if (checkExistence("v86-seiwonpark.iso")) {
    window.iso = "v86-seiwonpark.iso" + "?version=" + window.version;
    console.log("using " + window.iso + " as iso");
  } else {
    console.log("v86-seiwonpark.iso not found");
    window.iso = "";
  }
} else {
  if (window.initial_state == "" && checkExistence(window.params.get("iso"))) {
    //if it doesn't exist
...
if (window.params.has("autosave")) {
  ...
  window.persist = true;
  if (
    typeof (window.params.get("autosave") == "number") &&
    window.params.get("autosave") > 0
  ) {
    window.autosaveTime = window.params.get("autosave");
    console.log(
      "using " + window.autosaveTime + "seconds as autosave interval"
    );
  } else {
...
function loadSaves() {
  if (emulator.is_running()) {
    ...
    localforage
      .getItem("snapshot-" + window.params.get("iso"))
      .then(function (value) {
        if (value != null) {
          state = value;
          ...
          document.getElementById("save_time").innerHTML =
            "restored from save at " + getTimestamp();
...
function save() {
  emulator.save_state(function (error, new_state) {
    ...
    state = new_state;
    document.getElementById("save_time").innerHTML =
      "saved to temporary at " + getTimestamp();
  });
}
...
function restore() {
  emulator.restore_state(state);
  document.getElementById("save_time").innerHTML =
    "restored from temporary at " + getTimestamp();
}
...
async function startAutosave(auto) {
  if (!window.autosave_lock) {
    ...
    localforage
      .setItem("snapshot-" + window.params.get("iso"), temp_state)
      .then(function () {
    ...
    if (auto) {
      document.getElementById("save_time").innerHTML =
        "autosaved at " + getTimestamp();
    } else {
      //just a normal save
      document.getElementById("save_time").innerHTML =
        "saved at " + getTimestamp();
      console.log("saved");
    }
  }
}
...
function checkForUpdates() {
  ...
  if (current_version != null) {
    if (current_version < window.version) {
      ...
      if (
        window.confirm(
          "There is a newer version of the rootfs available. Delete all data and reload Now?"
        )
      ) {
        delete_data();
        ...
        alert(
          "You will no longer be receiving any rootfs updates - Delete save to opt back in"
        );
      }
    } else {
...
function checkExistence(filename) {
  ...
  http.open(
    "HEAD",
    window.location.href.split("?")[0].replace("index.html", "") + filename,
    false
  );
  http.send();
  if (http.status == 200) {
